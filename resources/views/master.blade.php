...
<head>
    ...
    <style>
        body {
            background: {{ (option_get('bg_color') !== false) ? option_get('bg_color') : env('SCRIPT_BACKGROUND_COLOR', '#000') }} 
        }
    </style>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark" style="background:{{ option_get('navbar_color') !== false ? option_get('navbar_color') : env('SCRIPT_PRIMARY_COLOR', '#000') }} ">
        <div class="container">
            ...
            <form action="{{ route('search') }}" method="get" class="form-inline my-2 my-lg-0">
                ...
                <button class="btn {{ env('SCRIPT_BUTTON_COLOR') }} my-2 my-sm-0 border-radius-2"  type="submit">{{ $text_setting[1] != "" ? $text_setting[1] : "ค้นหา" }}</button>
            </form>
            </div>
        </div>
    </nav>
    ...
    <script>
        $(document).ready(function(){
            $(".ads-click").click(function(){
                ...
                $.ajax({
                    url: "{{ url('api/v1/ads-click/') }}/"+data_id,
