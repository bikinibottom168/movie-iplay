...
@elseif($movie->type == "series" || $movie->type == "anime")
    ...
    <div class="player_container" id="player_movie" style="display: {{ $ads_movie_video_count == 0 ? 'block' : 'none' }};">
        <div style="display: block;" class="movie_player">
            ...
            @endphp
            <iframe src="{{ $movie_array_for_index[0] != "" ? $movie_array_for_index[0] : '' }}" id="player_iframe" style="width: 100%; height: 230px; border: none;/* position: relative; z-index: 2147483647*/"
...
@elseif($movie->type == "series")
    @php
        // $count_filename = (array) json_decode($movie->file_series);
        // $count_filename = count($count_filename);
    @endphp
        {{ $first = true }}
        ...
        <div class="episode" style="padding: 0 20px;color: #fff;text-align:center;">
            @if($first == true)
                <div class="episode_path"  data-ep-name="{{ $movie->title }} - {{ $source }}" data-href="{{ $source }}" style="width: 100%; background-color: #127ba3;cursor: pointer;padding: 10px;border-radius: 5px;margin: 5px">
                    <i class="fas fa-play"></i> {{ $movie->title }} - {{ $source }}
                </div>
                {{ $first = false }}
            @else
                <div class="episode_path"  data-ep-name="{{ $movie->title }} - {{ $source }}" data-href="{{ $source }}" style="width: 100%; background-color: #5b5b5b;cursor: pointer;padding: 10px;border-radius: 5px;margin: 5px">
                    <i class="fas fa-play"></i> {{ $movie->title }} - {{ $source }}
                </div>
            @endif
        </div>
