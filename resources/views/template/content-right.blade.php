<div class="row">
    ...
    <div class="col-lg-20 col-20 mt-1">
        <div class="widget" style="background: {!! option_get('primary_color') != null ? option_get('primary_color') : "#f2f4f5;" !!}">
            @foreach ($ads_r2 as $k)
                <a href="{{ get_url_ads($k) }}" target="_blank" class="ads-click" data-id="{{ $k->id }}">
                    <img src="{{ asset($k->image_ads) }}" alt="{{ $k->title_ads }}" style="width: 100%">
                </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="col-lg-20 col-20 mt-4">
        <div class="widget" style="background: {!! option_get('primary_color') != null ? option_get('primary_color') : "#f2f4f5;" !!}">
            <div class="widget-title text-dark">
                <p><span>{{ $text_setting[2] != "" ? $text_setting[2] : "หมวดหมู่" }} <i class="fa-solid fa-clapperboard"></i></span></p>
            </div>
    ...
    @if(env('SCRIPT_TYPE', '') == "movie" || env('SCRIPT_TYPE', '') == "anime")
        @if(count($playlist) != 0)
            <div class="col-lg-20 col-20 mt-4">
                <div class="widget" style="background: {!! option_get('primary_color') != null ? option_get('primary_color') : "#f2f4f5;" !!}">
                    <div class="widget-title text-dark">
                        <p><span>{{ $text_setting[13] != "" ? $text_setting[13] : "หนังไตรภาค" }}</span></p>
                    </div>
                    <ul class="categorys" style="padding-right: 5px">
    ...
    <div class="col-lg-20 col-20 mt-4">
        <div class="widget" style="background: {!! option_get('primary_color') != null ? option_get('primary_color') : "#f2f4f5;" !!}">
            <div class="widget-title text-dark">
                <p><span>{{ $text_setting[3] != "" ? $text_setting[3] : "ปีหนัง" }} <i class="fa-solid fa-calendar"></i></span></p>
            </div>
