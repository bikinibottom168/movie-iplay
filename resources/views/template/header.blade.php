    ...
    <div class="container-fluid">
      ...
      <div class="navbar-collapse collapse">
          ...
          <form action="{{ route('search') }}" method="get" class="form-inline mt-2 mt-md-0">
              ...
              <button type="submit" class="btn btn-outline-light my-2 my-sm-0">{{ __('site.search') }}</button>
          </form>
      </div>
    </div>
</nav>
        ...
        <div class="header-right">
            ...
            <div class="header-search">
                <form action="{{ route('search') }}" method="get">
                    ...
                    <input type="text" name="title" class="header-search-input" placeholder="{{ __('site.search') }}">
                    <button type="submit" class="header-search-button">{{ __('site.search') }}</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="navbar">
        <ul class="nav">
            <li class="nav-main-item  menu-item menu-item-type-custom menu-item-object-custom current-menu-item current_page_item menu-item-home menu-item-28">
                <a class="nav-main-link" href="{{ route('home') }}" style="font-size: 19px">{{ __('site.home') }}</a>
            </li>
            @foreach ($category_menu as $k)
                <li class="cat-item">
