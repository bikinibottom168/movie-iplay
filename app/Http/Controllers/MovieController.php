...
class MovieController extends Controller
{
  ...
  public function Main()
  {
    ...
    $data['category'] = genre::join('categorys_movies', 'genres.id', '=', 'categorys_movies.category_id')
      ...
      {
          $data['text_setting'] = array("ดูหนังออนไลน์",'ค้นหา',"หมวดหมู่","ปีหนัง","ตัวอย่าง","หนังอื่นๆ","Director","Actors","Genres","เรื่องย่อ","แสดงเพิ่ม","หนังเรื่องอื่นๆ","แชร์ให้เพื่อนดูด้วย","หนังไตรภาค");
      }
      else {
          $data['text_setting'] = unserialize(option_get("text_setting"));
      }


    $data['playlist'] = Playlist::orderBy('title', 'ASC')->get();

    $data['movie_hot'] = movie::orderBy('id','desc')->limit(16)->get();
    // $data['text_setting'] = option_get('text_setting');
  ...
  public function index(Request $request)
  {
    ...
    $data['description'] = $data['setting']->description;
    $data['news'] = Article::orderBy('updated_at', 'desc')->select('id','title','slug_title','view','created_at','updated_at','image')->paginate(12);

    $data['movie'] = movie::orderBy('created_at', 'desc')
  ...
  // Streaming MP4 JWPLAYER
  public function streaming_test()
  {

    $data = $this->Main(); // Main มาใช้
    $sources = "https://streaming-3.vip-streaming.com:30443/customer99/3-ssd/movie/store-2000/%E0%B8%81%E0%B9%89%E0%B8%B2%E0%B8%A7%E0%B8%99%E0%B8%B5%E0%B9%89%E0%B9%80%E0%B8%9E%E0%B8%B7%E0%B9%88%E0%B8%AD%E0%B9%80%E0%B8%98%E0%B8%AD%E0%B8%9E%E0%B8%B2%E0%B8%81%E0%B8%A2%E0%B9%8C%E0%B9%84%E0%B8%97%E0%B8%A2-EP01.mp4/playlist.m3u8";
    $data['url'] = $sources;
    ...
    if (env('VIDEO_PLAYER', 'jwplayer') == "videojs") {
      ...
      player.setup({
          ...
          //   "schedule": [
          //       {
          //           "tag": "http://red168ball.com/ads/overlay.xml"
          //       }    
          //   ]
          // }
        });';
    ...
    return view('template.movie.streaming_test', $data);
  }
  ...
  public function get_cloudstream($url, $includ_env = false)
  {
    ...
    $response = $client->request('GET', "https://client.iamtheme.com/api/v1/token/".config('app.token')."/".$ip."?salt=".$salt,['http_errors' => false]);
    $get = json_decode($response->getBody(), true);
    $token = $get['data'];

    // GET STREAMING PLAYLIST
    $video_url = $url;
    ...
    if ($includ_env) {
        ...
        $check_http = strpos($url, "ssd/");
          ...
          if($check_http === false)
          {
            $check_http = strpos($url, "av/japan");
          }
          
          if($check_http === false)
          {
            $check_http = strpos($url, "anime/");
          }

          if ($check_http === false) {
            if(env("stream_config","0") == "0")
            {
              $video_url = base64_decode(env('STREAMING_ID_URL', '')).$url."/playlist.m3u8";
            }
            else
            {
              $video_url = base64_decode(config('app.STREAMING_ID_URL')) . $url . "/playlist.m3u8";
            ...
            {
              $video_url = base64_decode(config('app.STREAMING_ID_URL_SSD')) . $url . "/playlist.m3u8";
  ...
  public function movie($title)
  {
    ...
    Cookie::queue('movie_id',$data['movie']->id);
