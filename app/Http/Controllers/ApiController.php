...
class ApiController extends Controller
{
    ...
    public function api($token_hash)
    {
        ...
        $client = new Client;
        $response = $client->request('GET', $domain.'api/v1/movie/iamapi353890',['http_errors' => false]);
        ...
        foreach($get->data as $k)
        {
            ...
            if($check->count() >= 1)
            {
                ...
                // {
                    ...
                    if($check_version->image != $k->image)
                    {
                        ...
                        // if ($webp->save(public_path($k->image))) {
            ...
            // UPDATE CATEGORY
            $this->api_category('iamapi353890');
    ...
    public function api_category($token_hash)
    {
        ...
        $client = new Client;
        $response = $client->request('GET', $domain.'api/v1/movie-category/iamapi353890',['http_errors' => false]);
        $get = json_decode($response->getBody());
    ...
    public function ads_click($id)
    {
        // Check data
        $data_ads = ads::find($id);
        $data_ads->count_click = $data_ads->count_click + 1;
        $data_ads->save();
    }
